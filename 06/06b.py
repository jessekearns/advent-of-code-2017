...
while True:
    ...
    steps += 1
    matched = False
    for arrIndex in range(0, len(previousArrangements)):
        arrangement = previousArrangements[arrIndex]
        listMatched = True
        for m in range(0, len(blocks)):
            listMatched = listMatched and (blocks[m] == arrangement[m])
        if listMatched :
            matched = listMatched
            print("Match found at index {0}".format(arrIndex))
            loopFirstIndex = arrIndex
            break
    if (matched):
        print(blocks)
        print(steps)
        print("Loop is length {0}".format(str(steps - loopFirstIndex)))
        break