...
def GetLongestBridge(current, matrix, bridgeSoFar, strengthSoFar):
    candidates = GetNextComponentCandidates(current, matrix)
    if len(candidates) == 0:
        if (len(bridgeSoFar) >= 40): # Ran it, plugged in the highest I saw, repeat
            # print(bridgeSoFar)
    ...
    for candidate in candidates:
        ...
        candidateBridge = GetLongestBridge(nextConnection, matrix, bridgeSoFar + [(current, nextConnection)], strengthSoFar + candidateStrength)
        matrix[current][nextConnection] += 1
        matrix[nextConnection][current] += 1
        candidateLength = len(candidateBridge)
...
bridge = GetLongestBridge(0, componentMatrix, [], 0)
