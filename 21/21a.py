...
# Irrelevant because I misunderstood but I'm keeping this around
def quarterMatrix(matrix):
    size = len(matrix[0])
    halfSize = int(size/2)
    topLeft = []
    topRight = []
    ...
    for i in range(halfSize):
        ...
        for j in range(halfSize):
            topLeft[i].append(matrix[i][j])
            topRight[i].append(matrix[i][j + halfSize])
            bottomLeft[i].append(matrix[i + halfSize][j])
            bottomRight[i].append(matrix[i + halfSize][j + halfSize])
